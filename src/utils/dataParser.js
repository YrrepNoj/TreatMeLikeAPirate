         ...
         // console.log("going to location")
          ...
          if(pieces[3].length > 0){
            ...
            if(splitLocation[3] == "N" || splitLocation[3] == "S"){
              var lat = ConvertDMSToDD(splitLocation[1], splitLocation[2], 0, splitLocation[3]);
              var lng = ConvertDMSToDD(splitLocation[4], splitLocation[5], 0, splitLocation[6]);
              var toDecimal = [lat, lng];
              pieces[3] = toDecimal;
            }else if(splitLocation[4] == "N" || splitLocation[4] == "S"){
              var lat = ConvertDMSToDD(splitLocation[1], splitLocation[2], splitLocation[3], splitLocation[4]);           
              var lng = ConvertDMSToDD(splitLocation[5], splitLocation[6], splitLocation[7], splitLocation[8]);
              var toDecimal = [lat, lng];
              pieces[3] = toDecimal;
            }else{
